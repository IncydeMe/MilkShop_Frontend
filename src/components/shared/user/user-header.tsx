"use client"
import { BookUser, ShoppingCartIcon, TicketPercent, UserCircle2, ReceiptText, Gift } from 'lucide-react';
    ...
    const CartItem: React.FC<CartItemProps> = ({
        ...
        return (
            <li className='flex items-center justify-between w-full'>
                <img src={item.product.imageUrl} alt={item.product.name} className='w-[60px] h-[60px] object-cover rounded-[4px]' />
                <div className='flex flex-col gap-2'>
                    <p>{item.product.name}</p>
                    <p>{item.quantity} x {item.product.price}đ</p>
                </div>
            </li>
        )
    }
